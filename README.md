<!-- ════════════════════════════════════════════════════════════════════════════
     COOPERATIVE POLAR-OCEAN OBSERVATION SYSTEM — README / BLUEPRINT
     Version 2.1 · 2026
     Single source of truth for the project. Generated with:
       · 30 Python/matplotlib figures (scripts/gen_figures*.py → assets/figures/)
       · 34 Mermaid diagrams (rendered inline by GitHub / VS Code / Typora)
       · 15 Google-sourced illustrative photographs (assets/images/, credited in §20.5)
     ════════════════════════════════════════════════════════════════════════════ -->

# 🌊 Cooperative Polar-Ocean Observation System

> **An indigenous, two-device autonomous ocean-observation mission for the Southern Ocean.**
> A wave-powered surface **Wave Glider** and a diving **Argo Float** work as one cooperative
> system — predicting their own drift with machine learning, meeting at the surface every
> ~10 days to hand over data and recharge, avoiding icebergs autonomously, and streaming the
> polar ocean and atmosphere to a live mission dashboard at NCPOR.

| | |
|---|---|
| **Project** | Cooperative Polar-Ocean Observation System |
| **Concept** | Wave Glider × Argo Float · cooperative polar mission |
| **Status** | 🟡 Concept & feasibility — Version 2.1 · 2026 · Pre-implementation |
| **Audience** | NCPOR scientists, engineers, reviewers, developers, operators |
| **Source** | `Polar_Ocean_Observation_System_Overview.pdf` (47 pp., v1.0, 2026) |
| **License** | TBD — internal to NCPOR until a publication strategy is approved |
| **Repo layout** | [§17 Repository Structure](#17-repository-structure-blueprint) |

**Badges (fill in once CI and hosting exist):**

```text
[![docs](https://img.shields.io/badge/docs-blueprint-v2.0-blue)]
[![build:server](https://img.shields.io/badge/build-server-passing-brightgreen)]
[![build:dashboard](https://img.shields.io/badge/build-dashboard-passing-brightgreen)]
[![ml:validation](https://img.shields.io/badge/ml-validation-passing-brightgreen)]
[![status](https://img.shields.io/badge/status-concept%20%26%20feasibility-yellow)]
```
...
This README is the **canonical blueprint and single source of truth** for the project.
It is written so that **no prior knowledge of ocean robots or machine learning is assumed**.

| If you have… | Read… |
|---|---|
| **5 minutes** | [§1 Executive Summary](#1-executive-summary) + the architecture figure in [§4](#4-system-architecture-and-end-to-end-flow) + the mission-cycle figure in [§6](#6-one-mission-cycle-dive-predict-meet-recharge) |
| **30 minutes** | All of Part I (Sections 1–16) — the complete system design |
| **A day** | The whole document, including the engineering blueprint (Sections 17–18) and the appendices |
| **A specific question** | The [Table of Contents](#table-of-contents) below or the [traceability matrix](#appendix-a-traceability-to-the-concept-document) |

### Document conventions

| Convention | Meaning |
|---|---|
| 💬 *In plain language* | Non-technical explanation of the preceding technical content |
| ⚠️ **Engineering honesty** | A deliberate statement of a limitation or hard constraint the design must respect |
| 🛡️ **Fail-safe** | How the system degrades safely when things go wrong |
| 📝 **Design note** | A design decision that is not yet an engineering commitment |
| 🧪 **Validation** | How a claim is tested before it is trusted |
| `code` | Commands, file names, identifiers, message fields |
| *Illustrative* | Values that show the **form** of an output — not measured or committed figures |
| 🖼️ Figure N | Python/matplotlib-generated blueprint figure in `assets/figures/` |
| 📷 Plate | Google-sourced illustrative photograph in `assets/images/` (credited in §20.5) |

### The four media in this document

1. **Text & tables** — the specification itself.
2. **🖼️ Python figures** — 30 figures generated by `scripts/gen_figures.py` (figures 1–24) and `scripts/gen_figures_v3.py` (figures 25–30)
   (matplotlib, deterministic, reproducible from source).
3. **Mermaid diagrams** — 34 live diagrams rendered by GitHub/VS Code/Typora from inline
   source (architecture, sequence, state, class, ER, quadrant, timeline, mindmap).
4. **📷 Photographs** — representative imagery of the vehicle types and environment,
   used for educational purposes (sources in §20.5).

> ℹ️ **Why both Mermaid and Python figures?** Mermaid gives you *editable, version-controllable*
> flowcharts that render inside GitHub without any tooling. The Python figures give you
> *quantitative* visuals — timelines, profile plots, risk matrices, Gantt charts — that Mermaid
> cannot express. Edit the Mermaid in the markdown; regenerate the PNGs with
